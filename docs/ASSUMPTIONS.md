...
## 13. Live validation with Gemini (25 Sep 2026)

- **Setup**: a Google AI Studio key with billing. `gemini-2.5-flash` is not available to new keys (404), so extraction fell back automatically to `gemini-3.8-flash`, the newest GA Flash model. Busy-model errors (429, 500, 503, 504) are retried three times with growing pauses (a 429 waits the delay the server asks for) before the next GA Flash model is tried; a model found unavailable is not asked again in the same process.
- **Extraction**: 38 PDFs (14 case documents, 24 PDFs of test set v2; the UBL e-invoice and the email-body invoice need no model). All 684 compared fields match the ground truth (identifiers normalised as the gate uses them, amounts to the cent). No critical field fell below the 0.80 threshold. Report: `docs/LIVE_VALIDATION.md`, regenerated by `make validate` from the cache, without calling the API.
- **Ground truth correction**: the Nordwind account statement (v2 document 3) prints a balance only. Gemini returned no net total, which is right, so the fixture now has `net_total: null` for statements.
- **Scans**: the fixtures give the two scans simulated confidences (section 12). The real model read both correctly and with high confidence. So in to-be the degraded scan (v2 document 13, an 86.43 EUR non-PO invoice) is posted through DoA auto-approval instead of going to human review. This is the one gate result that differs from `tests/golden_v2.yaml`, which describes fixture mode. The confidence gate still sends a document to human review whenever the model itself reports low confidence.
- **Owner-message drafts**: checked live for the four to-be exceptions of the case documents (B-05 missing receipt, B-06 price variance, B-08 wrong entity, B-12 quantity variance). Each names the right owner, supplier, invoice number, amount, PO and SLA date. They are cached in `data/cache/drafts/`.
- **Cost**: at Google's list price of 25 Sep 2026 for gemini-3.8-flash (0.75 USD input and 3.75 USD output per million tokens, thinking included), the 38 extractions cost about 0.20 USD, roughly half a US cent per invoice.
